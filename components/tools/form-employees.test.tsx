...
vi.mock("./employee-salary-field", () => ({
  default: ({ employeeName }: { employeeName: string }) => (
...
function FormEmployeesHarness() {
  ...
  return <FormEmployees register={register} t={mockTranslator} />;
}
...
describe("FormEmployees", () => {
  it("renders the expected employee salary fields", () => {
    ...
    expect(
      screen.getByTestId("employee-field-deliveryDriver"),
    ).toBeInTheDocument();
    expect(screen.getByTestId("employee-field-hrManager")).toBeInTheDocument();
    expect(
      screen.getByTestId("employee-field-logisticsManager"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("employee-field-purchasingAgent"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("employee-field-factoryWorker"),
    ).toBeInTheDocument();
  });
